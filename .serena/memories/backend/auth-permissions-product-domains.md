...
- Lockout and RPC rate limits share the `app.http.errors/handle-error :rate-limit` HTTP path: status 429, body `{:type :rate-limit :code ... :hint ... :ttl ...}`, and any supplied `::http/headers` preserved. A non-nil `:ttl` (seconds) adds `retry-after`; account lockout raises `:code :account-locked` while the RPC limiter raises `:code :request-blocked` and keeps `x-rate-limit-remaining`/`x-rate-limit-reset`. The external flag is `enable-account-lockout`; external RPC limiter flag is `enable-rpc-rlimit`.
