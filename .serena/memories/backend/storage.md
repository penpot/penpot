...
- Recording never fails: `app.metrics/run-safe!` replaces raw `run!` at every emit point (`emit-op!`, `emit-dedup!`, `emit-asset!`, and the three S3 publisher emissions). It no-ops on a nil metrics instance; the first failure per `hint` logs at `warn`, subsequent failures at `debug` (one surfacing per emit site, no log flood). The `publish` outer try/catch stays: it is an SDK `MetricPublisher` contract boundary, not a metrics guard.
