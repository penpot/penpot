...
- Physical S3 API calls (AWS SDK `MetricPublisher`, `app.storage.s3.metrics`):
  - `penpot_storage_s3_requests_total{operation,target,result}` — one count per API call (`PutObject`, `GetObject`, `HeadObject`, `DeleteObject(s)`, …), including retries. `result` is `"ok"` only when the SDK reports success as exactly `true`; a missing success flag counts as `error`.
