...
// FIXME: Looks like we could benefit from splitting this large file into multiple
// specs files, and have a TokensPage that extends WasmWorkspacePage to deal
...
test.describe("Tokens: Tokens Tab", () => {
  ...
  test("User creates color token and auto created set show up in the sidebar", async ({
    ...
    await expect(
      tokensTabPanel.getByRole("checkbox", {
        name: "color.primary",
      }),
    ).toBeEnabled();
    ...
    await expect(
      tokensTabPanel.getByRole("checkbox", { name: "#ff0000" }),
    ).toHaveCount(2);
...
test.describe("Tokens: Apply token", () => {
  test("User applies color token to a shape", async ({ page }) => {
    ...
    await tokensSidebar
      .getByRole("checkbox", { name: "colors.black" })
      .click({ button: "right" });
