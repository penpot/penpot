...
test.describe("Tokens: Themes modal", () => {
  test("Delete theme", async ({ page }) => {
    ...
    await expect(
      tokenThemeUpdateCreateModal.getByTestId("delete-theme"),
    ).toHaveCount(2);

    await tokenThemeUpdateCreateModal
      .getByTestId("delete-theme")
      .first()
      .click();
  ...
  test("Edit theme", async ({ page }) => {
    ...
    await expect(tokenThemeUpdateCreateModal.getByText("3 SETS")).toHaveCount(
      2,
    );

    await tokenThemeUpdateCreateModal.getByTestId("edit-theme").first().click();
