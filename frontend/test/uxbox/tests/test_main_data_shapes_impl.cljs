...
(t/deftest drop-shape-test1
  (let [initial {:pages {1 {:id 1 :shapes [1 2 3]}}
                 :shapes {1 {:id 1 :page 1}
                                2 {:id 2 :page 1}
                                3 {:id 3 :page 1}}}
        expected (assoc-in initial [:pages 1 :shapes] [3 1 2])
...
(t/deftest drop-shape-test5
  (let [initial {:pages {1 {:id 1 :shapes [1 4]}}
        ...
        expected (-> initial
                     ...
                     (assoc-in [:shapes 1 :items] [2])
                     (update-in [:shapes 3] dissoc :group))
        result (impl/drop-shape initial 3 4 :after)]
    (t/is (= result expected))
    (t/is (vector? (get-in result [:pages 1 :shapes])))))
...
(t/deftest drop-shape-test8
  (let [initial {:pages {1 {:id 1 :shapes [1 5 6]}}
                 :shapes {1 {:id 1 :page 1
                                ...
                                6 {:id 6 :page 1}}}
        expected (-> initial
                     (assoc-in [:pages 1 :shapes] [1 5])
                     (update-in [:shapes 2 :items] conj 6)
                     (update-in [:shapes 6] assoc :group 2))
...
(t/deftest drop-shape-test9
  (let [initial {:pages {1 {:id 1 :shapes [1]}}
        ...
        expected (-> initial
                     (assoc-in [:pages 1 :shapes] [1])
                     (assoc-in [:shapes 2 :items] [3 4 6])
                     (assoc-in [:shapes 1 :items] [2 5])
                     (update-in [:shapes 6] assoc :group 2))
        result (impl/drop-shape initial 6 4 :after)]
    ;; (pprint expected)
    ;; (pprint result)
