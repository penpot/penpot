...
The tooltip requires an `id` prop, which is used to associate the trigger with the tooltip content via the `aria-describedby` attribute. This ensures screen readers can announce the tooltip content when the trigger is focused.
...
A small delay is applied before showing the tooltip to avoid accidental triggers, the duration of this delay is 300ms.
