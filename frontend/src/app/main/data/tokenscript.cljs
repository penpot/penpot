(ns app.main.data.tokenscript
  (:require
   ...
   ["@tokens-studio/tokenscript-interpreter" :refer [BaseSymbolType
    ...
    :as tokenscript]
   ["@tokens-studio/tokenscript-interpreter$errorClasses" :refer [DependencyError]]
   ["@tokens-studio/tokenscript-interpreter$linter" :refer [LintRunner
                                                            TypeBasedRule]]
...
(defn dependency-error? [err]
  (instance? DependencyError err))
...
;; Linter ----------------------------------------------------------------------
;; The tokenscript linter is a tool to write custom verification logic for tokens
;; Lint rules can be defined per token type

(defn expect-number-symbol
  [value]
  (when-not (or (number-symbol? value) (number-with-unit-symbol? value))
    :error.token/expect-number-symbol))

(defn expect-safe-int
  [value]
  (when (or (>= (.-value value) sm/max-safe-int)
            (<= (.-value value) sm/min-safe-int))
    :error.token/number-too-large))

(def allowed-units #{"%" "px" "rem"})

(defn expect-allowed-units
  [value]
  (when (number-with-unit-symbol? value)
    (let [unit (.-unit value)]
      (when-not (contains? allowed-units unit)
        :error.token/disallowed-unit))))

(defn expect-valid-numberic [value context create-issue]
  (when-let [error-code (or
                         (expect-number-symbol value)
                         (expect-allowed-units value)
                         (expect-safe-int value))]
    (create-issue context value error-code)))

(def rules
  (-> (TypeBasedRule.)
      (.forType "border-radius" expect-valid-numberic)))
