...
import { isColorFill, isLinearGradient, isRadialGradient, isAngularGradient, isDiamondGradient, isImageFill } from './constants'
...
export function addFillProps(
  ...
  const hasGradientFill = shapeFills.some(
    (fill: Fill) => isLinearGradient(fill) || isRadialGradient(fill) || isAngularGradient(fill) || isDiamondGradient(fill)
  )
  const urlFillNeeded =
    hasImageFill ||
    hasGradientFill ||
  ...
  if (shapeFills.length > 0) {
    ...
    } else if (isLinearGradient(firstFill) || isRadialGradient(firstFill) || isAngularGradient(firstFill) || isDiamondGradient(firstFill)) {
