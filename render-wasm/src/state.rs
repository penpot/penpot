...
impl State {
    ...
    pub fn rebuild_tiles_shallow(&mut self) -> Result<()> {
        self.render_state.rebuild_tiles_shallow(&self.shapes)
    }

    pub fn rebuild_tiles(&mut self) -> Result<()> {
        self.render_state.rebuild_tiles_from(&self.shapes, None)
    }

    pub fn rebuild_tiles_from(&mut self, base_id: Option<&Uuid>) -> Result<()> {
        self.render_state.rebuild_tiles_from(&self.shapes, base_id)
    }

    pub fn rebuild_touched_tiles(&mut self) -> Result<()> {
