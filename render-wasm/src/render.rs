...
impl RenderState {
    ...
    /// Fast pan/zoom preview: compose cached GPU content and present immediately.
    ///
    /// Draws the doc atlas as a full-viewport backdrop, then overlays cached tile
    /// textures on top (without clearing again) so HQ tiles replace the low-res
