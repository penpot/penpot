...
thread_local! {
    pub static STATE: RefCell<Option<Box<State<>>>> = const { RefCell::new(None) };
}
...
pub extern "C" fn init(width: i32, height: i32) {
    STATE.with(|state| {
        ...
        if state.is_none() {
            *state = Some(Box::new(State::new(width, height, 2048)));
        }
    });
}
...
pub extern "C" fn set_render_options(debug: u32, dpr: f32) {
    STATE.with(|state| {
        if let Some(state) = state.borrow_mut().as_mut() {
            state.render_state.set_debug_flags(debug);
            state.render_state.set_dpr(dpr);
        }
    });
}
...
pub extern "C" fn set_view(zoom: f32, x: f32, y: f32) {
    STATE.with(|state| {
        if let Some(state) = state.borrow_mut().as_mut() {
            state.render_state.invalidate_cache_if_needed();
...
pub extern "C" fn set_shape_masked_group(masked: bool) {
    STATE.with(|state| {
        if let Some(state) = state.borrow_mut().as_mut() {
            state.with_current_shape(|shape| shape.set_masked(masked));
        }
    });
}
...
pub extern "C" fn set_shape_bool_type(raw_bool_type: u8) {
    STATE.with(|state| {
        if let Some(state) = state.borrow_mut().as_mut() {
            state.with_current_shape(|shape| shape.set_bool_type(BoolType::from(raw_bool_type)));
        }
    });
}
...
pub unsafe extern "C" fn set_shape_type(shape_type: u8) {
    STATE.with(|state| {
        if let Some(state) = state.borrow_mut().as_mut() {
            state.with_current_shape(|shape| shape.set_shape_type(Type::from(shape_type)));
        }
    });
}
...
pub extern "C" fn set_shape_selrect(left: f32, top: f32, right: f32, bottom: f32) {
    STATE.with(|state| {
        if let Some(state) = state.borrow_mut().as_mut() {
            state.with_current_shape(|shape| shape.set_selrect(left, top, right, bottom));
        }
    });
}
...
pub extern "C" fn set_shape_clip_content(clip_content: bool) {
    STATE.with(|state| {
        if let Some(state) = state.borrow_mut().as_mut() {
            state.with_current_shape(|shape| shape.set_clip(clip_content));
        }
    });
}
...
pub extern "C" fn set_shape_rotation(rotation: f32) {
    STATE.with(|state| {
        if let Some(state) = state.borrow_mut().as_mut() {
            state.with_current_shape(|shape| shape.set_rotation(rotation));
        }
    });
}
...
pub extern "C" fn set_shape_transform(a: f32, b: f32, c: f32, d: f32, e: f32, f: f32) {
    STATE.with(|state| {
        if let Some(state) = state.borrow_mut().as_mut() {
            state.with_current_shape(|shape| shape.set_transform(a, b, c, d, e, f));
        }
    });
}
...
pub extern "C" fn add_shape_child(a: u32, b: u32, c: u32, d: u32) {
    STATE.with(|state| {
        if let Some(state) = state.borrow_mut().as_mut() {
            let id = uuid_from_u32_quartet(a, b, c, d);
            state.with_current_shape(|shape| shape.add_child(id));
        }
    });
}
...
pub extern "C" fn clear_shape_children() {
    STATE.with(|state| {
        if let Some(state) = state.borrow_mut().as_mut() {
            state.with_current_shape(|shape| shape.clear_children());
        }
    });
}
...
pub extern "C" fn add_shape_solid_fill(raw_color: u32) {
    STATE.with(|state| {
        if let Some(state) = state.borrow_mut().as_mut() {
            let color = skia::Color::new(raw_color);
            state.with_current_shape(|shape| shape.add_fill(shapes::Fill::Solid(color)));
        }
    });
}
...
pub extern "C" fn add_shape_linear_fill(
    ...
    STATE.with(|state| {
        if let Some(state) = state.borrow_mut().as_mut() {
            state.with_current_shape(|shape| {
...
pub extern "C" fn clear_shape_fills() {
    STATE.with(|state| {
        if let Some(state) = state.borrow_mut().as_mut() {
            state.with_current_shape(|shape| shape.clear_fills());
        }
    });
}
...
pub extern "C" fn clear_shape_strokes() {
    STATE.with(|state| {
        if let Some(state) = state.borrow_mut().as_mut() {
            state.with_current_shape(|shape| shape.clear_strokes());
        }
    });
}
...
pub extern "C" fn clear_shape_shadows() {
    STATE.with(|state| {
        if let Some(state) = state.borrow_mut().as_mut() {
            state.with_current_shape(|shape| shape.clear_shadows());
        }
    });
}
