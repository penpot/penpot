...
pub fn render_shape_enter(
    ...
    if let Type::Group(group) = element.shape_type {
        ...
        if group.masked {
            ...
            render_state.surfaces.canvas(target_surface).save_layer(&layer_rec);
        }
    }
    ...
    if mask {
        ...
        render_state.surfaces.canvas(target_surface).save_layer(&mask_rec);
    }
