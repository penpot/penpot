...
impl Gradient {
    ...
    fn to_radial_shader(&self, rect: &math::Rect) -> skia::Shader {
        ...
        let scale_factor = distance / (rect.height() / 2.);
        // We need an extra transform, because in skia radial gradients are circular and we need them to be ellipses
        transform.pre_scale(
            (
                scale_factor * rect.width() / 2. / distance,
                scale_factor * rect.height() / 2. / distance,
            ),
            None,
        );
