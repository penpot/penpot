...
impl TextContentSize {
    ...
    pub fn copy(&mut self, size: TextContentSize) {
...
impl TextContent {
    ...
    fn text_layout_auto_width(&self) -> TextContentLayoutResult {
        ...
        let width = paragraphs
            .iter()
            .flatten()
            .fold(0.0, |auto_width, paragraph| {
                f32::max(paragraph.max_intrinsic_width(), auto_width)
            });

        let size = TextContentSize::new(width.ceil(), f32::INFINITY, width.ceil());
    ...
    pub fn update_layout(&mut self, selrect: Rect) -> TextContentSize {
        ...
        match self.grow_type() {
            GrowType::AutoHeight => {
                let result: TextContentLayoutResult = self.text_layout_auto_height();
                self.set_layout_from_result(result);
            }
            GrowType::AutoWidth => {
                let result: TextContentLayoutResult = self.text_layout_auto_width();
                self.set_layout_from_result(result);
            }
            GrowType::Fixed => {
                let result: TextContentLayoutResult = self.text_layout_fixed();
                self.set_layout_from_result(result);
            }
        }
        self.size.clone()
