...
impl From<[u8; RAW_PARAGRAPH_DATA_SIZE]> for RawParagraphData {
    fn from(bytes: [u8; RAW_PARAGRAPH_DATA_SIZE]) -> Self {
        Self {
            text_align: bytes[1],
            text_direction: bytes[2],
            text_decoration: bytes[3],
            text_transform: bytes[4],
            line_height: f32::from_be_bytes([bytes[5], bytes[6], bytes[7], bytes[8]]),
            letter_spacing: f32::from_be_bytes([bytes[9], bytes[10], bytes[11], bytes[12]]),
            typography_ref_file: [
                u32::from_be_bytes([bytes[13], bytes[14], bytes[15], bytes[16]]),
                u32::from_be_bytes([bytes[17], bytes[18], bytes[19], bytes[20]]),
                u32::from_be_bytes([bytes[21], bytes[22], bytes[23], bytes[24]]),
                u32::from_be_bytes([bytes[25], bytes[26], bytes[27], bytes[28]]),
            ],
            typography_ref_id: [
                u32::from_be_bytes([bytes[29], bytes[30], bytes[31], bytes[32]]),
                u32::from_be_bytes([bytes[33], bytes[34], bytes[35], bytes[36]]),
                u32::from_be_bytes([bytes[37], bytes[38], bytes[39], bytes[40]]),
                u32::from_be_bytes([bytes[41], bytes[42], bytes[43], bytes[44]]),
            ],
        }
    }
}
...
impl From<&Vec<u8>> for RawTextData {
    fn from(bytes: &Vec<u8>) -> Self {
        let num_leaves = bytes[0] as usize;
