...
use crate::math;
use super::{Type, Shape, StructureEntry, Rect, Path, Segment};
...
pub trait ToPath {
    fn to_path(
        ...
        structure: &HashMap<Uuid, Vec<StructureEntry>>,) -> Path;
}
...
fn make_corner(corner_type: CornerType, from: (f32, f32), to: (f32, f32), r: math::Point) -> Segment {
...
pub fn rect_segments(shape: &Shape, rect: &Rect) -> Vec<Segment> {
    let sr = shape.selrect;

    if let Some([r1, r2, r3, r4]) = rect.corners {
...
const BEZIER_CIRCLE_C: f32 = 0.551915024494;
...
impl ToPath for Shape {
    fn to_path(
        ...
        match shape.shape_type {
            ...
            Type::Rect(ref rect) => {
                Path::new(rect_segments(&shape, rect))
            }
            Type::Path(path_data) => {
                path_data
            }
            Type::Circle => {
                Path::new(circle_segments(&shape))
            }
