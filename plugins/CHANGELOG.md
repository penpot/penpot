...
- **plugin-types:** Added optional per-side stroke widths (`strokePerSide`, `strokeWidthTop`, `strokeWidthRight`, `strokeWidthBottom`, `strokeWidthLeft`) to the `Stroke` interface, so plugins can read and write the per-side widths of boards and rectangles. Related to #11804.
- **plugins-runtime:** Strokes read through `Shape.strokes` now preserve per-side mode and widths when committed back, so changing an unrelated stroke property no longer collapses them to the uniform `strokeWidth`. Related to #11804.
