---
description: Investigate the chosen task, produce an implementation plan, and save it
agent: build
---
...
1. **Produce the plan** with the `planner` skill. By default, research the
   ...
   example, `delegated` in the arguments), delegate to the `general` subagent
...
3. Once all decisions are answered and the plan is final, save it verbatim to the
   ...
   exist). This step is this command's explicit authorization to write the plan
...
While this command runs, act read-only: research with read-only tools only.
