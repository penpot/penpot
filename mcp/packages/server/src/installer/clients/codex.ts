...
export const codex: ClientInstaller = {
    id: "codex",
    label: "Codex CLI",
    describe() {
        return "Codex CLI. Delegates to `codex mcp add` when the binary is on PATH.";
    },
    configPath() {
    ...
    snippet(opts: InstallOptions) {
        return {
            command: `codex mcp add ${opts.entryName} --transport http ${opts.serverUrl}`,
        };
    },
