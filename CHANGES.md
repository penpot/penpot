...
- Fix lost-update race in `team.features` during concurrent `create-file` RPC calls. Two simultaneous file-creation requests on the same team would each read `team.features` at the start of the transaction, each compute the union with their own client-declared features, and each write back — with the second commit silently overwriting the first's appended feature. The team-row mutation is now wrapped in a `SELECT ... FOR UPDATE` against the existing `::db/transaction true` envelope and the diff is re-derived against the locked snapshot, so concurrent waiters serialise correctly and no no-migration / team-inheritable feature is dropped [Github #9197](https://github.com/penpot/penpot/issues/9197)
