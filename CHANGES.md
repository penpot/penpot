...
- Fix plugin API `Shape.component()` returning the OUTERMOST component for shapes inside a nested copy — calling `.component()` on a button inside a `related-card` wrapper now correctly resolves to the button's own component instead of the wrapping `related-card` [Github #9183](https://github.com/penpot/penpot/issues/9183)
