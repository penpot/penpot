...
- Fix plugin API `shape.fills` and `shape.strokes` arrays being read-only: per-element writes (`fills[0].fillColor = ...`), nested gradient edits, element replacement (`fills[0] = {...}`), and `push` / `pop` now commit to shape state instead of silently failing [Github #8357](https://github.com/penpot/penpot/issues/8357)
